...
class SidebarButton(QPushButton):
    def __init__(self, icon, text, count, active=False, active_style="type", parent=None):
        super().__init__(parent)
        self.icon = icon
        self.text_text = text
        self.count = count
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        for key, color, label in [
            ...
            icon = "●" if color else "📁"
            btn = SidebarButton(icon, label, 0, active=key == "all", active_style="status")
            if color:
                btn.setStyleSheet(btn.styleSheet().replace("color: #475569", f"color: {color}").replace("color: #0f172a", f"color: {color}"))
